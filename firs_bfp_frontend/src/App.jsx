...
function LoginPage({ onLogin, onForgot }) {
  ...
  async function doLogin() {
    ...
    try {
      ...
      if (!res.ok) { setErr(data.message || "Incorrect username or password."); return; }
    ...
    <div className="auth-wrap">
      <div className="auth-card">
        ...
        <div className="auth-body">
          ...
          <p className="auth-sub">Enter your registered BFP email address and we'll send you a verification code.</p>
          {err && <div className="auth-err">{err}</div>}
          <div className="auth-fg">
...
export default function App() {
  ...
  useEffect(() => { if (token) setPage("app"); }, []);

  useEffect(() => { if (page === "app" && token) fetchIncidents(); }, [page, token]);
  ...
  function handleLogin(tok, display) {
    ...
    setToken(tok); setDispName(display);
    setPage("app"); setActiveView("dashboard");
  }

  function doLogout() {
    sessionStorage.removeItem("firs_token");
    sessionStorage.removeItem("firs_display");
    setToken(""); setIncidents([]); setPage("login");
  }

  function confirmLogout() {
    showConfirm("Log Out", "Are you sure you want to log out of FIRS?", async () => {
      try { await fetch(`${API_BASE}/logout/`, { method:"POST", headers: authHeaders(token) }); } catch {}
      doLogout();
  ...
  async function handleSave(rec) {
    try {
      ...
      const url     = editRec ? `${API_BASE}/incidents/${editRec.id}/` : `${API_BASE}/incidents/`;
  ...
  function handleDelete(rec) {
    showConfirm("Delete Incident",
      ...
      async () => {
        try {
          await fetch(`${API_BASE}/incidents/${rec.id}/`, { method:"DELETE", headers: authHeaders(token) });
          await fetchIncidents();
  ...
  function handleImport(recs, onDone) {
    showConfirm("Confirm Import",
      `Import ${recs.length} record(s) into the system?`,
      async () => {
        try {
